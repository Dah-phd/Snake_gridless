...
class snake:
    ...
    def draw_snake(self, the_grid):
        ...
        if self.speed > 0:
            ...
            while len(self.position) > self.tail*15/self.speed:
                del self.position[-1]
        ...
        if self.turn and self.turn[-1][1] == self.position[-2][0] and self.turn[-1][2] == self.position[-2][1]:
            del self.turn[-1]
        if self.turn and self.turn[-1][1] == self.position[-1][0] and self.turn[-1][2] == self.position[-1][1]:
            del self.turn[-1]
        # it will give u head
        if self.facing == 'up':
            headposition = head
    ...
    def control_snake(self):
        ...
        if key_dir[pygame.K_RIGHT] and self.facing != 'left' and self.facing != 'right':
            if not self.turn or abs(self.y - self.turn[0][2]) > 22:
        ...
        if key_dir[pygame.K_UP] and self.facing != 'down' and self.facing != 'up':
            if not self.turn or abs(self.x - self.turn[0][1]) > 22:
    ...
    def colision(self, the_grid):
        if self.speed != 0:
            speed_adj = int(75/self.speed)
        else:
            speed_adj = 25
        for t in self.position[speed_adj:-2]:
